(() => {
  ...
  function getViewFromQuery() {
    ...
    if (query.includes('driver')) return 'driver';
    if (query.includes('config')) return 'config';
    return 'display';
  }

  function setupViewMode() {
    const view = getViewFromQuery();
    const mapping = {
      display: { viewId: 'displayView', title: 'PIS · Fahrgastanzeige' },
      driver: { viewId: 'driverView', title: 'PIS · Fahrersteuerung' },
      config: { viewId: 'configView', title: 'PIS · Konfiguration' },
    };

    document.querySelectorAll('.app-view').forEach((element) => {
      element.classList.add('is-hidden');
    });
    document.getElementById(mapping[view].viewId)?.classList.remove('is-hidden');

    const title = document.getElementById('appTitle');
    if (title) title.textContent = mapping[view].title;
  ...
  function occupancyLabel(value) {
    ...
    if (value === 'very-high') return 'sehr hoch';
    return 'normal';
  }
  ...
  function render(config, state) {
    ...
    document.getElementById('occupancyChip').textContent = `Auslastung: ${occupancyLabel(state.occupancyLevel)}`;
    document.getElementById('simChip').textContent = state.simulationEnabled
      ? `Simulation: aktiv (${state.simulationIntervalSeconds}s)`
      : 'Simulation: pausiert';

    const serviceNotice = document.getElementById('serviceNotice');
    const notice = String(state.serviceNotice || '').trim();
    serviceNotice.textContent = `Servicehinweis: ${notice || '—'}`;
    serviceNotice.classList.toggle('is-hidden', !notice);
  ...
  (async function init() {
    const currentView = setupViewMode();
    ...
    function scheduleSimulationTick() {
      const intervalMs = Math.max(5, Number(state.simulationIntervalSeconds) || 15) * 1000;
      setTimeout(() => {
        if (state.simulationEnabled) {
          ...
          if (route?.stops?.length) {
            ...
            saveState(state);
            if (currentView === 'display') render(config, state);
          }
        }
